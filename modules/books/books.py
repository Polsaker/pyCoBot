...
class books:
    ...
    def book(self, bot, cli, event):
        ...
        r = urllib.request.urlopen("https://www.googleapis.com/books/v1/"
            "volumes?q={0}&langRestrict={2}&country=AR&key={1}&maxResults=3".format(stext, self.apikey, lang)).read()
        ...
        for b in search['items']:
            ...
            try:
                resp += "Autor(es): \002{0}\002. ".format(", ".join(b['volumeInfo']['authors']))
            except:
                pass
            try:
                resp += "Publicación: \002{0}\002. ".format(b['volumeInfo']['publishedDate'])
            except:
                pass
            try:
